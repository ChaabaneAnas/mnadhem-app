...
function VariantModal({
  ...
  return (
    <Modal title={isEdit ? t('variantModalEdit') : t('variantModalAdd')} onClose={onClose}>
      <FormError message={error} />
      <form action={handleSubmit} className="space-y-3">
        <div>
        ...
        {!isEdit && (
          <div>
            ...
            <p className="mt-1 text-xs text-muted-foreground">{t('initialCountHint')}</p>
          </div>
        )}
        <div className="flex justify-end gap-2 pt-1">
          <Button type="button" variant="outline" size="sm" onClick={onClose} className="rounded-md border-border text-xs text-muted-foreground hover:bg-muted hover:text-muted-foreground transition-colors">
          ...
          <Button type="submit" size="sm" disabled={pending} className="rounded-md text-xs font-medium disabled:opacity-50 transition-colors">
            {pending ? tc('saving') : isEdit ? tc('saveChanges') : t('addVariantBtn')}
          </Button>
        </div>
      </form>
    </Modal>
    ...
    <Sheet open onOpenChange={(open) => { if (!open) onClose(); }}>
      ...
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-border shrink-0">
          <SheetTitle className="text-sm font-semibold text-foreground">{t('bulkTitle')}</SheetTitle>
          <Button variant="ghost" size="icon-sm" onClick={onClose} className="text-muted-foreground hover:bg-transparent hover:text-foreground transition-colors">
            <X size={16} />
          </Button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          ...
          <div className="rounded-lg bg-muted border border-border p-4">
            <p className="text-xs font-medium text-foreground mb-2">{t('expectedFormat')}</p>
          ...
          {rows.length > 0 && (
            <div>
              <div className="flex items-center justify-between mb-2">
                <p className="text-xs font-medium text-foreground">{t('rowsParsed', { count: rows.length })}</p>
                {errorCount > 0 && (
                  <span className="text-xs text-red-600 dark:text-red-400 font-medium">{t('rowsWithErrors', { count: errorCount })}</span>
                )}
              </div>
                ...
                <table className="w-full text-xs">
                  <thead>
                    <tr className="bg-muted">
                      {[t('colHash'), t('colName'), t('colSku'), t('colVariantSku'), t('priceHeader'), t('colInWarehouse'), ''].map((h, i) => (
                    ...
                    {rows.map((row) => (
                      ...
                      >
                        ...
                        <td className="px-3 py-2">
                          {row.errors.length > 0 && (
                            <span className="text-red-500">{row.errors.map((c) => t(c)).join(', ')}</span>
                          )}
                        </td>
                      </tr>
                    ))}
        ...
        <div className="shrink-0 px-5 py-4 border-t border-border flex justify-end gap-2">
          ...
          >
            {pending ? t('importing') : t('importBtn', { count: validRows.length })}
          </Button>
        </div>
      </SheetContent>
    </Sheet>
        ...
        <div className="mb-6 flex items-start justify-between">
          <div>
            ...
            <p className="mt-0.5 text-sm text-muted-foreground">
              {t('summary', { products: products.length, variants: totalVariants })}
              {lowStock > 0 && (
                <span className="ms-2 text-red-600 dark:text-red-400 font-medium">{t('outOfStock', { count: lowStock })}</span>
              )}
            </p>
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-muted border-b border-border">
                  <th className="px-4 py-3 text-start text-xs font-medium text-muted-foreground uppercase tracking-wide">{t('colProductVariant')}</th>
                  <th className="px-4 py-3 text-start text-xs font-medium text-muted-foreground uppercase tracking-wide">{t('colSku')}</th>
                  <th className="px-4 py-3 text-end text-xs font-medium text-muted-foreground uppercase tracking-wide">{t('colInWarehouse')}</th>
                  <th className="px-4 py-3 text-end text-xs font-medium text-muted-foreground uppercase tracking-wide">{t('colCommitted')}</th>
                  <th className="px-4 py-3 text-end text-xs font-medium text-muted-foreground uppercase tracking-wide">{t('colReadyToSell')}</th>
                  ...
                  <>
                    ...
                    <tr key={`p-${product.id}`} className="border-b border-border bg-muted/60">
                      <td colSpan={5} className="px-4 py-2.5">
                        ...
                        {product.sku && <span className="ms-2 text-xs text-muted-foreground font-mono">{product.sku}</span>}
                      </td>
                    ...
                    {product.variants.map((variant) => (
                      <tr key={variant.id} className="border-b border-border hover:bg-muted/50">
                        <td className="ps-8 pe-4 py-2.5 text-sm text-muted-foreground">{variant.name}</td>
                        <td className="px-4 py-2.5 text-xs text-muted-foreground font-mono">{variant.sku ?? '—'}</td>
                        <td className="px-4 py-2.5 text-end text-sm text-foreground tabular-nums">{variant.stockPhysical}</td>
                        <td className="px-4 py-2.5 text-end text-sm text-foreground tabular-nums">{variant.stockReserved}</td>
                    ...
                    {product.variants.length === 0 && (
                      <tr key={`empty-${product.id}`}>
                        <td colSpan={6} className="ps-8 px-4 py-2.5 text-xs text-muted-foreground italic border-b border-border">
                          {t('noVariants')} <Button variant="link" onClick={() => setAddVariantFor(product)} className="h-auto p-0 text-xs text-muted-foreground underline hover:text-foreground">{t('addOne')}</Button>
                        </td>
                      </tr>
                    )}
                  </>
