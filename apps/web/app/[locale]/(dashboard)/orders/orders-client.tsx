    ...
    <Sheet open onOpenChange={(open) => { if (!open) onClose(); }}>
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-border shrink-0">
          <SheetTitle className="text-sm font-semibold text-foreground">{t('newOrder')}</SheetTitle>
          <Button variant="ghost" size="icon-sm" onClick={onClose} className="text-muted-foreground hover:bg-transparent hover:text-foreground transition-colors">
            <X size={16} />
          </Button>
        </div>
        ...
        <form id="create-order-form" onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-5 py-4 space-y-5">
          {/* ── Customer ── */}
          <div>
            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-3">{t('customer')}</p>
          ...
          {/* ── Items ── */}
          <div>
            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-3">{t('items')}</p>

            {/* Variant search */}
            <div className="relative mb-3">
              <Search size={13} className="absolute start-3 top-1/2 -translate-y-1/2 text-muted-foreground pointer-events-none z-10" />
              <Input
                value={search}
                  ...
                  {filteredVariants.map((v) => (
                    ...
                    >
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">{v.name}{v.sku && <span className="ms-1 font-mono text-muted-foreground">{v.sku}</span>}</p>
                      </div>
                      <div className="text-end shrink-0 ms-3">
                        ...
                        <p className={`text-xs ${v.stockAvailable > 0 ? 'text-green-700 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                          {t('ready', { count: v.stockAvailable })}
                        </p>
                      </div>
                    </Button>
                  ))}
                ...
                <Table className="text-xs">
                  <TableHeader>
                    <TableRow className="bg-muted hover:bg-muted">
                      <TableHead className="px-3 py-2 text-start font-medium text-muted-foreground h-auto">{t('colItem')}</TableHead>
                      <TableHead className="px-3 py-2 text-end font-medium text-muted-foreground h-auto">{t('colPrice')}</TableHead>
                      <TableHead className="px-3 py-2 text-end font-medium text-muted-foreground h-auto">{t('colQty')}</TableHead>
                      <TableHead className="px-3 py-2 text-end font-medium text-muted-foreground h-auto">{t('colTotal')}</TableHead>
                  ...
                  <TableBody>
                    {lineItems.map((item) => (
                      <TableRow key={item.variantId} className="border-t border-border">
                        ...
                        <TableCell className="px-3 py-2 text-end text-foreground tabular-nums">{formatTND(item.price)}</TableCell>
          ...
          {lineItems.length > 0 && (
            <div>
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-3">{t('review')}</p>
              <div className="rounded-lg border border-border bg-muted p-4 space-y-3">
                <div>
                  <Label className="mb-1">{t('orderReference')}</Label>
                  <Input
                    value={reference}
                ...
                <div className="flex items-center justify-between pt-1 border-t border-border">
                  <span className="text-xs font-medium text-muted-foreground">{t('totalCod')}</span>
            ...
            <Table className="text-sm">
              <TableHeader>
                <TableRow className="bg-muted border-b border-border hover:bg-muted">
                  {columns.map((col) => (
                    <TableHead key={col.key} className="px-4 py-3 text-start text-xs font-medium text-muted-foreground uppercase tracking-wide last:w-20 h-auto">
                      {col.label}
                    </TableHead>
                  ))}
                </TableRow>
                  ...
                  return (
                    <TableRow key={order.id} className="border-b border-border hover:bg-muted/50">
                      ...
                      <TableCell className="px-4 py-3">
                        <span className={`inline-flex items-center rounded-full border px-2 py-0.5 text-xs font-medium ${orderStatusClasses(order.status)}`}>
                          {ts(order.status)}
                        </span>
                      </TableCell>
