...
export class VariantsService {
  ...
  async adjustStock(id: string, dto: StockAdjustmentDto, tenantId: string) {
    ...
    if (newPhysical < 0) {
      throw new BadRequestException('Adjustment would result in negative physical stock');
    }
    if (newPhysical < variant.stockReserved) {
      throw new BadRequestException(
        `Cannot reduce physical stock below reserved quantity (${variant.stockReserved})`,
      );
    }
  ...
  async remove(id: string, tenantId: string) {
    ...
    if (variant.stockReserved > 0) {
      throw new ConflictException(
        `Cannot archive "${variant.name}" — ${variant.stockReserved} unit(s) are currently with a courier. Wait for those shipments to complete first.`,
      );
    }
